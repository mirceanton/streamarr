...
func BulkJobsSeriesHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, ep := range episodes {
		...
		ops, skipReason := buildEpisodeOps(ep, keepAudio, keepSub, extractSub)
		if len(ops) == 0 {
...
func buildEpisodeOps(ep models.MediaFile, keepAudio, keepSub, extractSub map[string]bool) ([]models.Operation, string) {
